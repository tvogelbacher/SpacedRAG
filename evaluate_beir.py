...
parser.add_argument('--corpus_chunk_size', type=int, default=50000, help='Number of samples to be encoded at once. Reduce if you run out of memory.')
...
if 'contriever' in args.model_code:
    ...
    model = DRES(DenseEncoderModel(encoder, doc_encoder=encoder, tokenizer=tokenizer), batch_size=args.per_gpu_batch_size, corpus_chunk_size=args.corpus_chunk_size)
...
#    model = DRES(DPR((model_code_to_qmodel_name[args.model_code], model_code_to_cmodel_name[args.model_code])), batch_size=args.per_gpu_batch_size, corpus_chunk_size=5000)
    ...
    model = DRES(models.SentenceBERT(model_code_to_cmodel_name[args.model_code]), batch_size=args.per_gpu_batch_size, corpus_chunk_size=args.corpus_chunk_size)
else:
